...
1. 不知道放哪的内容，先写到 `01-收件箱/`。
2. 先读 `AGENTS.md`，但不要随意修改它。
3. 不要把密码、token、SSH 私钥、公司隐私信息写进仓库。
4. `01-收件箱/` 默认是未审材料，不是正式知识区。
...
然后随便新建一条测试笔记，例如在 `01-收件箱/` 下写一个测试文件，再执行：
...
如果你要改主要知识区，先确认自己是在整理、修订，而不是随手覆盖别人的内容。
